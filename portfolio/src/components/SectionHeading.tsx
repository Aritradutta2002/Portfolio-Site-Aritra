...
/* Shared editorial section header — mono index in acid, oversized
   title, hairline rule. One component so all sections match. */
export function SectionHeading({ index, eyebrow, title, blurb, align = 'left' }: Props) {
  ...
  return (
    ...
    >
      <p className={`font-mono text-xs uppercase tracking-[0.25em] text-acidstrong mb-4 ${centered ? '' : ''}`}>
        <span className="text-ink/40">{index}</span>
        <span className="mx-3 text-ink/20">/</span>
        {eyebrow}
      </p>
      <h2 className="font-bold tracking-[-0.03em] leading-[1.02] text-ink text-4xl sm:text-5xl lg:text-6xl text-balance" aria-label={title}>
      ...
      <div className={`mt-8 h-px w-full bg-line/10 ${centered ? '' : ''}`} aria-hidden="true" />
    </motion.div>
  )
}
