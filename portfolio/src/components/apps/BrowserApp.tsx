...
export default function BrowserApp() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#f5f7fb] text-[#1d2433] dark:bg-[#1e1e20] dark:text-[#f4f4f6]">
      <form onSubmit={openAddress} className="flex items-center gap-2 border-b border-black/[0.08] bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-[#252529]">
        <Globe2 size={16} className="text-[#4c73d8]" />
        <div className="flex flex-1 items-center gap-2 rounded-full bg-[#eef1f7] px-3 py-2 dark:bg-[#35353b]">
          <Search size={14} className="text-black/35 dark:text-white/45" />
          <input value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Search or enter a website address" aria-label="Website address" className="w-full bg-transparent text-[12px] outline-none placeholder:text-black/35 dark:placeholder:text-white/45" />
        </div>
